...
% Author:
% Modifications:Jonathan Menéndez Cardona, 18023
...
WHEEL_RADIUS = 0.032;
MAX_SPEED = WHEEL_RADIUS * MAX_WHEEL_VELOCITY*1.2;
ell=96/2000; 
r=32/2000;
MAX_CHANGE = 0.001;  % rad/s
% goals = webots_path; %[- 0.8, 0.8;-0.6, 0.6; -0.4, 0.4; -0.2, 0.2; 0, 0;0.2, -0.2];

%% Obtener todos los sensores del Pololu 3pi+

% Pose
robotat = robotat_connect();
agent = 7; % Select pololu robot available
con_agent = robotat_3pi_connect(agent);
%orientation_sensor = robotat_get_pose(robotat,agent,'eulxyz');
...
if graph_type == "grid"
    pos = robotat_get_pose(robotat,7,'eulzyx');
...
pos = robotat_get_pose(robotat,agent,'eulxyz');
xi = pos(1);  zi = pos(2);
trajectory = [xi, zi];
v_hist = [];
w_hist = [];
...
while 1
    ...
    north = pos(6);
    bearing_north = bearing_deg(7);
%     rad = atan2(north(1), north(1));
%     
%     if rad < 0
%        rad = rad + 2*pi; 
%     end
...
%         fprintf(formatSpec, xi, zi, eP, theta, theta_g, eO);
    ...
    theta = north - bearing_north;  % Se corrige el �ngulo para que est� igual que theta_g
...
%     end
    ...
    disp(left_speed)
    disp(right_speed)
    robotat_3pi_set_wheel_velocities(con_agent,left_speed,right_speed);
